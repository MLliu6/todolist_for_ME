...
> 一个保护注意力的本地优先任务系统：从今天出发，快速捕获、明确下一步，再用日历和四象限完成规划。

## ✨ 功能特性

- ☀️ **Today-first 首页** — 打开后直接看到下一件事、今日节奏、逾期压力、本周完成率与已估时投入
- ⚡ **5 秒快速捕获** — 首页输入一句话即可添加今天/明天的任务，支持 `/` 聚焦和 `Ctrl/Cmd + K` 命令中心
- 🎯 **下一步建议** — 综合 DDL、逾期、进行中状态与四象限，给出透明且可控的单项建议
- ⏱️ **轻量任务估时** — 任务可选 15–120 分钟投入，用于今日容量反馈，不做黑箱自动排程
- ♻️ **可恢复归档** — Burnout 减速归档的任务会进入首页归档箱，可随时恢复
- 📆 **日历主视图** — 支持月度 / 周度 / 日度切换（已移除小时视图，避免伪精细化）
- 🎯 **四象限法** — 可视化象限选择器（右上=紧急重要，左上=不紧急但重要，右下=紧急不重要，左下=不紧急不重要）
- 🏷️ **自定义标签** — 支持自定义 + 已有标签快速选用
- ⏰ **计划日 / DDL 分离** — “准备哪天做”进入 Today 与日历，“最晚哪天完成”只用于真实硬截止与逾期判断
...
- 🎨 **Atelier 视觉系统** — 暖中性纸张底色、墨色结构与珊瑚橙品牌强调，四象限继续使用独立语义色
- 📱 **响应式工作台** — 桌面使用安静的侧栏导航，移动端切换为五项底部主导航，高级视图仍可从首页进入
...
> 产品原则：高级视图服务于计划与复盘，不与“捕获 → 选择 → 执行”的每日主循环争夺注意力。
